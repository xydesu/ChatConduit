...
**ChatConduit** 是一款專為現代 Minecraft 伺服器設計的高效能、全功能多頻道聊天插件。具備全 GUI 箱子介面零指令操作系統、HikariCP 非同步資料庫連線池、Redis 跨服聊天與頻道邀請廣播、Discord 雙向溝通以及獨立 Webhook 整合功能。
...
| `/chatconduit help` | - | 顯示 ChatConduit 幫助選單 | `chatconduit.use` |
| `/chatconduit reload` | - | 重載插件設定與語言檔案 | `chatconduit.admin` |

### 特殊權限節點
- `chatconduit.chat.color` - 允許在發言對話中使用舊版 Legacy 顏色碼 (如 `&a`, `&c`, `&#RRGGBB`)。
...
mvn clean package
```
編譯完成後，輸出的 Jar 檔案位於 `target/ChatConduit-1.0.jar`。
